class DeyeOptimizerFlowCard extends HTMLElement {
  ...
  energyEntity(panel) {
    if (panel.energy_entity) return panel.energy_entity;
    return String(panel.entity || "").replace(/_input_power$/, "_energy_today");
  }
  energyValue(panel) {
    const entity = this.energyEntity(panel);
    if (!entity || entity === panel.entity) return null;
    const value = Number(this._hass?.states?.[entity]?.state);
    return Number.isFinite(value) ? value : null;
  }
  formatEnergy(value) { return `${value.toLocaleString(undefined, { maximumFractionDigits: 2 })} kWh`; }
  ...
  render() {
    ...
    const energyValues = panels.map((panel) => this.energyValue(panel)).filter((value) => value !== null);
    const todayEnergy = energyValues.reduce((sum, value) => sum + value, 0);
    ...
    const cards = panels.map((panel, index) => {
      ...
      return `<div class="optimizer" style="--color:${color}" data-entity="${this.escape(panel.entity)}" role="button" tabindex="0" title="Open history">
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      header{display:flex;justify-content:space-between;align-items:end;margin-bottom:clamp(5px,1.5cqw,12px);gap:8px}.heading{min-width:0}h2{margin:0;font-size:clamp(15px,4cqw,27px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.today{margin-top:2px;color:var(--secondary-text-color,#a9b7ca);font-size:clamp(9px,2.4cqw,14px)}.total{font-size:clamp(16px,4.5cqw,30px);font-weight:900;white-space:nowrap}
    ...
    </style><ha-card>${this.config.show_header ? `<header><div class="heading"><h2>${this.escape(this.config.title)}</h2>${this.config.show_today_energy && energyValues.length ? `<div class="today">${this.escape(this.config.today_energy_label)}: ${this.formatEnergy(todayEnergy)}</div>` : ""}</div>${this.config.show_total_power ? `<div class="total">${Math.round(total)} W</div>` : ""}</header>` : ""}<div class="grid ${this.config.orientation}">${cards}${empties}</div></ha-card>`;
    this.shadowRoot.querySelectorAll(".optimizer").forEach((optimizer) => {
      const open = () => this.openHistory(optimizer.dataset.entity);
      optimizer.addEventListener("click", open);
      optimizer.addEventListener("keydown", (event) => {
