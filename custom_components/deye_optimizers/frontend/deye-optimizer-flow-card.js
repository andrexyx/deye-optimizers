class DeyeOptimizerFlowCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...
      show_total_power: true, show_percent: true, show_empty_slots: false,
  ...
  render() {
    ...
    const cards = panels.map((panel, index) => {
      ...
      const filledCells = Math.round(Math.min(100, percent) / 100 * 21);
      const cells = Array.from({ length: 21 }, (_, cellIndex) =>
        `<i class="cell${cellIndex >= 21 - filledCells ? " filled" : ""}"></i>`
      ).join("");
      return `<div class="optimizer" style="--color:${color}">
        <div class="visual"><div class="solar"><div class="cells">${cells}</div><b>${this.escape(panel.name || `P${index + 1}`)}</b></div></div>
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      header{display:flex;justify-content:space-between;align-items:end;margin-bottom:clamp(5px,1.5cqw,12px);gap:8px}h2{margin:0;font-size:clamp(15px,4cqw,27px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.total{font-size:clamp(16px,4.5cqw,30px);font-weight:900;white-space:nowrap}
      ...
      .visual{height:76%;display:flex;justify-content:center;align-items:stretch;min-height:0}.solar{position:relative;height:100%;max-width:100%;aspect-ratio:${this.config.orientation === "landscape" ? "1.7/1" : "1/1.7"};overflow:hidden;border:1px solid #b7c7e8;border-radius:3px;background:#20395f}.cells{position:absolute;inset:2px;display:grid;grid-template-columns:repeat(${this.config.orientation === "landscape" ? 7 : 3},1fr);grid-template-rows:repeat(${this.config.orientation === "landscape" ? 3 : 7},1fr);gap:1px}.cell{border:.5px solid rgba(220,232,255,.85)}.cell.filled{background:var(--color);opacity:.9}.solar b{position:absolute;z-index:3;top:1px;left:50%;transform:translateX(-50%);max-width:90%;overflow:hidden;text-overflow:ellipsis;font-size:clamp(5px,2.3cqw,12px);background:#19304ccc;padding:0 2px;border-radius:2px;white-space:nowrap}.power{text-align:center;font-size:clamp(7px,4.2cqw,24px);font-weight:900;line-height:1.05;margin-top:2px}.power small{font-size:50%}.percent{text-align:center;font-size:clamp(6px,2.5cqw,14px);font-weight:700}
    ...
    </style><ha-card>${this.config.show_header ? `<header><h2>${this.escape(this.config.title)}</h2>${this.config.show_total_power ? `<div class="total">${Math.round(total)} W</div>` : ""}</header>` : ""}<div class="grid ${this.config.orientation}">${cards}${empties}</div></ha-card>`;
  }
}
